...
#				ax.set_xticklabels([])#
	...
	#			ax.set_yticklabels([])
			...
			if i == n_params-1:
				...
				p.plot_conditional(j, i, bins=20, cmap = plt.cm.gray_r)
			for m in modes:
				plt.errorbar(x=m['mean'][j], y=m['mean'][i], xerr=m['sigma'][j], yerr=m['sigma'][i], color = cch, linewidth = 4)
			#if i==j+1:
			if j==0:
			...
			plt.ylim((0,185))
			plt.xlim((0.0002,0.00135))
			#plt.savefig('cond_%s_%s.pdf' % (parameters[i], parameters[j]), bbox_tight=True)
		#	plt.close()
	plt.subplots_adjust(wspace=0, hspace=0, bottom = 0.2, top = 0.8, left = 0.2, right = 0.8)
